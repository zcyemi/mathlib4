...
/-!
# Triangle Similarity in Euclidean Spaces

This file contains theorems about similarity of triangles in Euclidean spaces,
particularly those involving angles.
...
-/
theorem similar_of_side_side (h_ne_ab : dist a b ≠ 0) (h_ne_bc : dist b c ≠ 0) (h_ne_ca : dist c a ≠ 0)
    (h_ne_ab' : dist a' b' ≠ 0) (h_ne_bc' : dist b' c' ≠ 0) (h_ne_ca' : dist c' a' ≠ 0)
    (h1 : dist a b * dist b' c' = dist b c * dist a' b')
    (h2 : dist b c * dist c' a' = dist c a * dist b' c') :
    ![a, b, c] ∼ ![a', b', c'] := by
  have h_pos1 : 0 < dist a b := by simp [dist_pos, h_ne_ab]
  have h_pos1' : 0 < dist a' b' := by simp [dist_pos, h_ne_ab']
  have h_pos2' : 0 < dist b' c' := by simp [dist_pos, h_ne_bc']
  have h_pos3' : 0 < dist c' a' := by simp [dist_pos, dist_comm, h_ne_ca']
  rw [← div_eq_div_iff (by positivity) (by positivity)] at h1 h2
  set r : ℝ := (dist a b / dist a' b') with hr
  have r_ne : 0 < r := by aesop
  apply similar_of_three_pos_dist_eq
  grind
...
theorem similar_of_angle_angle (h_not_col : ¬ Collinear ℝ {a, b, c})
  ...
  exact similar_of_side_side (by positivity) (by positivity) (by positivity)
    (by positivity) (by positivity) (by positivity) h_sin1 h_sin2
...
theorem Similar.angle_eq_all (h : ![a, b, c] ∼ ![a', b', c']) :
  ...
  have h2 := angle_eq_of_similar (Similar.comm_right h)
  rw [angle_comm, angle_comm a'] at h2
  have h3 := angle_eq_of_similar (Similar.comm_left h)
